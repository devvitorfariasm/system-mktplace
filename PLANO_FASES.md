...
| 2 | Fatia vertical: os cinco serviços no mínimo para `OrderCreated → CONFIRMED`, outbox + relay, reserva com `UPDATE ... RETURNING`, coletor de auditoria, `GET /audit/orders/{id}/timeline` em uma query | feita (ITs de fluxo completo e de concorrência; compose com os 5 serviços) |
...
- Outbox em todos os serviços (ADR 0002) e eventos fora de ordem tratados por transição implícita (ADR 0005).
